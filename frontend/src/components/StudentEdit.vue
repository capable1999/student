<template>
  <div class="container">
    <h1 class="title"><i class="fas fa-edit icon"></i>Edit Student</h1>
...
export default {
  setup() {
    ...
    const handleSubmit = async () => {
      ...
      try {
        await axios.put(`http://localhost:8080/students/${route.params.id}`, student.value);
    ...
    onMounted(async () => {
      if (route.params.id) {
        try {
          const response = await axios.get(`http://localhost:8080/students/${route.params.id}`);
          student.value = response.data;
        } catch (error) {
          errorMessage.value = 'Error fetching student data. Please try again.';
